...
| `HexToBytes` | `hex` | `const std::string&` | KeyManager | Hex-строка (должна быть чётной) |
...
Файл полностью переписан. Раньше содержал тесты вместо реализации. Теперь это полноценная **header-only** библиотека с безопасной конвертацией hex, затиранием памяти и локализованными именами токенов.